...
// Short code #2 for archive-meal_post.php

function meal_blog_page_shortcode( $atts ) {
    $output = '';
    $meal_loop_atts = shortcode_atts(
      [
        'type' => 'meal_post',
    ...
    $the_query = new WP_Query($args);
      	...
      	$output .= '</h2>';
		    while ($the_query->have_posts()) : $the_query->the_post();
		        $post_id = get_the_ID();
		      	$output .= '<div class="col-sm-12 meal-planner">';
		      		$output .= get_the_post_thumbnail($post_id, 'medium');
		      		$output .= '<article>';
		      			$output .= '<h3>';
		      				$output .= get_the_title();
    ...
    add_shortcode('meal-blog-loop', 'meal_blog_page_shortcode');
